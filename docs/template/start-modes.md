...
partial process set. A port collision is a blocked start; a missing executable,
immediate exit, unsolicited child signal, readiness error, or readiness timeout
is an unavailable dependency. Commands use argv arrays with the customer target
as their exact working directory. Logs are prefixed by process group and redact
bearer tokens and secret-like assignments.
...
The reviewed URL is `http://127.0.0.1:5173`; readiness is
...
reserves the local Convex ports `3210` and `3211` before spawning.
