...
| Dodo payments             | `DODO_API_KEY`, `DODO_WEBHOOK_SECRET`, `DODO_ENVIRONMENT`, `DODO_BUILD_PACK_PRODUCT_ID`, `DODO_BUILD_PACK_EXPECTED_AMOUNT_CENTS`, `DODO_BUILD_PACK_LAUNCH_CANARY`                                                                                      | Billing owner              | billing gateway, webhook verifier, product/amount binding, canary reconciliation | Test environment and fake keys create no real charges                                                                                                         | Required only for paid forks; webhook secret, product/amount binding, and idempotency checks must be live before billing launch   | Rotate API and webhook secrets before launch, after billing admin changes, and after webhook incidents   |
