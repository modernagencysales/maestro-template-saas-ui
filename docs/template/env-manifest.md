...
| WorkOS AuthKit            | `WORKOS_CLIENT_ID`, `WORKOS_ORGANIZATION_ID`, `WORKOS_REDIRECT_URI`, `WORKOS_LOGOUT_URI`, `WORKOS_COOKIE_PASSWORD`, `WORKOS_API_KEY`, `WORKOS_AUTHKIT_ISSUER`, `WORKOS_AUTHKIT_JWKS_URL` | Security owner          | TanStack Start auth shell, Convex auth bridge, membership provisioning   | Fake IDs, issuer, JWKS URL, and cookie password keep local demos non-live                                                                                                                | Required for production auth; redirect/logout URLs must match deployed domains; API key stays server-only; Convex trusts only the configured issuer/JWKS/client tuple | Rotate API key and cookie password before production, after access changes, and after any suspected leak          |
...
WorkOS forks derive Convex trusted JWT settings from `WORKOS_AUTHKIT_ISSUER`,
`WORKOS_AUTHKIT_JWKS_URL`, and `WORKOS_CLIENT_ID`. The template ships a
fake-safe explicit local mode; production startup rejects fake mode and missing
or malformed issuer/JWKS/client values before claiming live auth readiness.
