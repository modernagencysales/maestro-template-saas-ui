...
| WF-RETRY-INITIAL-BACKOFF | `graph.nodes[].retry.initialBackoffMs` | intentionally-restricted | V2 action retry is intentionally not compiled during WP-1.1. | Omit action retry until WP-1.2 maps the exact Convex retry options and dedupe proof. |
| WF-RETRY-BASE | `graph.nodes[].retry.base` | intentionally-restricted | V2 action retry is intentionally not compiled during WP-1.1. | Omit action retry until WP-1.2 maps the exact Convex retry options and dedupe proof. |
