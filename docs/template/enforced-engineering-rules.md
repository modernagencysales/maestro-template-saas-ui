...
| Product contract or runtime acceptance             | `pnpm check:product-contract`; `pnpm acceptance:required`                                                                                                      | Required revision-bound examples are selected and executed; their declared observations must pass. Causal strength and semantic test quality remain advisory review obligations. |
...
- `product.contract.yaml` is the customer-outcome authority. Do not add a
  parallel manifest, evidence store, or acceptance controller.
- `pnpm check:product-contract` validates the structural contract and
  `pnpm acceptance:required` selects and executes required revision-bound
  examples, requiring their declared observations to pass. Causal strength,
  declared-surface usefulness, and semantic test quality remain advisory review
  obligations. Fix flakes rather than masking them with retries or parallelism.
