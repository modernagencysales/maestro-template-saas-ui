...
| Advisory review       | An AI or human finding, plus Qlty after the composed active-authority change lands; it informs review but is not merge authority. |
...
- Qlty reports these eight authoring thresholds: identical code **12 lines**,
  similar code **15 lines**, function complexity **10**, file complexity **50**,
  return statements **5**, boolean logic **4**, function parameters **5**, and
...
- Current source staging is transitional: `.qlty/qlty.toml` sets analyzer
  `mode = "block"`, and the current Lefthook and Woodpecker firewall invoke
  `check:qlty`, whose exit status still propagates. The composed batch's
  active-authority change makes the wrapper advisory. Only after that change is
  integrated are Qlty findings advisory while Woodpecker remains the sole
  blocking CI authority.
- Dependency, secret, architecture, and Effect diagnostics have deterministic
  command owners below. After active-authority composition, Qlty's Gitleaks/OSV
  findings are advisory and do not replace the deterministic secret or
  dependency checks.
...
In the composed authority, Woodpecker is the sole blocking CI authority and
Qlty/AI remain advisory. The current source staging described above blocks on
Qlty until the active-authority wrapper change is integrated; do not
misrepresent either state or describe Qlty thresholds as Woodpecker blockers.
The detailed mapping of rules to mechanisms is in
[rule coverage](../rule-coverage.md), and examples and domain exceptions remain
