...
| Scheduled child workflow         | `runWorkflow` accepts scheduling fields, but 0.4.4 does not preserve the required child workflow options through the complete component path.                                                                | Unsupported on 0.4.4. Use an explicitly scheduled parent capability or upgrade only after matrix and behavior fixtures pass.                                             |
