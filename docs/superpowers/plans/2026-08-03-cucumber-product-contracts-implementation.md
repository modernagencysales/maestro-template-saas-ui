...
- **C3 legacy discovery reality:** the current repository has no pre-existing
  ...
  pretend those authorities exist or silently omit them. Discovery therefore
  uses the real route-tree `FileRoutesByFullPath`, typed Confect spec/AST
  manifests, all registered HTTP routes (including dynamic `/api/*`, static
  routes, and direct `POST /deploy-authority/consume`), the actual CLI/MCP
  registries, workflow publication registry, and raw Convex exports via the
  TypeScript checker/AST. For the one-time migration baseline only, the
  controller may synthesize deterministic IDs, coverage tags, and
  `auth_deny_all` records for discovered legacy authorities; the generated
  baseline records that synthesis and C4 rejects any post-baseline growth or key
  change without explicit registration. New code may not add a synthetic
  fallback. C3's Files and tests must include the real authority sources and
  prove the synthesized baseline is byte-stable.
...
### Task 3a: Normalize The Existing Confect Manifest Before Inventory

**Class:** `fixture-to-real` **PR:** `C3-pre` **Depends on:** `C2`

**Why this packet exists:** C3's deterministic discovery found 221 current
authorities and 22 contract operations, while the checked-in Confect manifest
contains only 8 operations. C3 must not hide that drift or claim a byte-stable
inventory over stale generated input.

**Files:**

- Modify only the existing Confect `*.spec.ts`/manifest source files that are
  proven to be missing from the reviewed generated set.
- Generate only the exact Confect manifest outputs produced by
  `pnpm confect:manifest` (record the generator output manifest and digests).
- Modify the existing manifest generator/tests only when required to make the
  deterministic source closure explicit; no public-surface inventory code is
  staged in this packet.

**Interfaces and constraints:**

- The normalized output must be a deterministic function of the sorted reviewed
  spec closure and current source; no hand-edited generated bytes.
- Every newly emitted operation must retain its real schema/ref/transport
  metadata. Missing or ambiguous refs fail closed; no synthetic product behavior
  is introduced.
- Runtime behavior must remain unchanged. UI/CLI/MCP/HTTP/workflow repairs
...
- [ ] **Step 1:** In a clean copy, run `pnpm confect:manifest`, capture the
      generated output manifest, and classify every added/removed operation and
      source file. Reject any output that cannot be traced to a reviewed spec.
- [ ] **Step 2:** Add/update only the missing reviewed specs or generator
      inputs, then rerun generation until the output is deterministic and
      source-complete. Add focused tests for duplicate IDs, missing refs, and
...
any remaining UI/CLI/MCP/HTTP/workflow authority gaps are explicit C3 inputs.
