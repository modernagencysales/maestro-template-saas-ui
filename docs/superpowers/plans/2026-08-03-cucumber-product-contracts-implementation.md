...
PR C1 pinned Cucumber/config
  ...
  -> C12 mutation gauntlet
       ...
       -> D1 old machinery deletion + alpha.3 seal
```
...
only external authority cutover. `M1` is a bottom-up sequence of ordinary GitHub
branches with the new journey assembling; `M2` is rebased onto current protected
main and contains the lifecycle flip plus a narrowly scoped integration repair
if required.
...
export type CheckedCucumberConfiguration = {
  ...
  readonly require: readonly ["features/**/*.ts"];
  readonly retry: 0;
  readonly parallel: 0;
};
...
- [ ] **Step 1: Write the failing configuration tests.** In
      `check-contracts.test.mts`, prove the exact four-key object passes and
      that `paths`, `tags`, `name`, `format`, `retry: 1`, `parallel: 1`, an
      altered TS loader, and every unknown key fail with the offending
      key/value.
...
- [ ] **Step 8: Commit generated and source authority together.**
  ...
  ```bash
  rtk git add tooling/confect-manifest packages/template-core/src/generated packages/template-core/package.json packages/convex/confect/http.ts packages/convex/convex/_generated/api.d.ts apps/web/src/navigation/reference-app-routes.ts apps/web/src/routeTree.gen.ts tooling/quality/check-system-topology.mts tooling/quality/check-system-topology.test.mts tooling/quality/check-headless-surface-contract.mts tooling/quality/check-headless-surface-contract.test.mts
...
- [ ] **Step 3: Compile with official Gherkin.** Use
      ...
      the byte-to-AST/Pickle pure compiler in the
...
- [ ] **Step 2: Write table-driven red mutations.** Starting from the passing
      ...
      linkage, orphan started/finished events, attempt > 0, `willBeRetried`,
      every non-PASSED status, failed run hook, unsuccessful/missing run finish,
      duplicate Pickle, wrong Source bytes/URI, and zero/wrong selection.
...
- [ ] **Step 4: Parse strictly before using `parseEnvelope`.** For each nonblank
      ...
      Index Sources, GherkinDocuments, Pickles, PickleSteps, TestCases,
      TestCaseStarted, step events, run hooks, attachments, and run boundaries
      by their runtime IDs. Reject duplicates and unresolved links at index
      time.
...
- [ ] **Step 5: Re-derive stable identity from runtime AST.** Follow Source ->
      ...
      per PickleStep, and passing scenario/run hooks.
...
- [ ] **Step 6: Validate the closed observation envelope.** Require exactly one
      ...
      step keys, and observed surface/transport pairs. The committed protocol
      fixture carries reviewed synthetic values matching its `RuntimeManifest`;
      `C9` replaces the fixture producer with trusted real drivers. Do not
      accept empty or extra fields.
...
- Create: `features/support/observations.test.ts`
...
- Create: `features/support/runtime-identity.test.ts`
...
- [ ] **Step 1: Write red observation tests.** Prove no public `markCovered`
      ...
      each scenario emits exactly one redacted attachment.
...
- [ ] **Step 3: Run red.**
  ...
  Run:
  `rtk host-test-slot --class focused pnpm exec vitest run features/support/observations.test.ts features/support/runtime-identity.test.ts apps/web/src/adapters/build-identity.test.ts packages/convex/test/runtime-identity.test.ts`
...
- [ ] **Step 5: Bind hooks to expected steps.** `Before` allocates scenario
      ...
      envelope. Global hooks perform checks only and never own long-lived
      processes.
...
- [ ] **Step 8: Run green and commit.**
  ...
  Run:
  `rtk host-test-slot --class focused pnpm exec vitest run features/support/observations.test.ts features/support/runtime-identity.test.ts apps/web/src/adapters/build-identity.test.ts apps/cli/src/commands.test.ts packages/convex/test/runtime-identity.test.ts tooling/acceptance/verify-messages.test.mts`

  ```bash
  rtk git add features/support apps/web/src/adapters/build-identity.ts apps/web/src/adapters/build-identity.test.ts apps/web/vite.config.ts apps/cli/src/commands.ts apps/cli/src/commands.test.ts packages/convex/confect/runtime packages/convex/confect/http.ts packages/convex/test/runtime-identity.test.ts tooling/acceptance/verify-messages.mts tooling/acceptance/verify-messages.test.mts
...
- Create: `features/support/local-auth.test.ts`
...
- [ ] **Step 3: Run red.**
  ...
  Run:
  `rtk host-test-slot --class focused pnpm exec vitest run tooling/agent-pack/src/processSupervisor.test.ts tooling/acceptance/sandbox.test.ts tooling/acceptance/run-acceptance.test.mts features/support/local-auth.test.ts`
...
- [ ] **Step 7: Orchestrate the authoritative sequence.** Verify checkout SHA
      ...
      all candidate processes; then verify Messages and runtime manifest. A
...
- [ ] **Step 1: Write the admitted Feature and undefined steps first.** Keep
      ...
      do not add this fake domain journey to normal customer output.
...
- [ ] **Step 2: Run dry-run red.**
  ...
  Expected: FAIL or report undefined snippets for every unimplemented step.
...
- [ ] **Step 1: Write a red completeness test.** Assert the mutation registry
      ...
      exist; Task 20 requires the complete 28-case registry before sealing.
...
- [ ] **Step 8: Run green and commit.**
  ...
  ```bash
  rtk git add packages/app-idea-evaluator/src packages/convex/confect/buildPacks packages/convex/confect/tables/buildPacks.ts packages/convex/confect/tables/buildPackStages.ts packages/convex/test/build-pack-pipeline.test.ts apps/web/src/features/public-funnel/build-pack
...
- Modify: `apps/cli/src/factory/composition.ts`
- Create generated target file: `.maestro/contracts-legacy-baseline.json`
...
- [ ] **Step 2: Write red atomic-apply tests.** Simulate collision, stale
      ...
      interruption, and unavailable target. Each must leave original files byte
      identical. Success stages config/guards/baseline, proves every old
      entrypoint remains available, then swaps as one transaction.
...
- [ ] **Step 3: Run red.**
  ...
  Run:
  `rtk host-test-slot --class focused pnpm --dir apps/cli exec vitest run src/factory/upgrade.test.ts`

  Expected: FAIL because current upgrade is plan-only and has no contracts
  audit.

- [ ] **Step 4: Extend the staged upgrade contract.** Add
      `maestro upgrade --contracts-audit` to the existing handler. Generate the
      content-addressed baseline from the pre-guard exhaustive inventory, stage
      pinned Cucumber/config/registration/guards in the current collision-free
      transaction, run static checks in the staged root, and atomically apply
      only when legacy reachability is unchanged.
...
- [ ] **Step 6: Run green and commit.**
  ...
  ```bash
  rtk git add tooling/release/src/upgrade apps/cli/src/factory/upgrade.ts apps/cli/src/factory/upgrade.test.ts apps/cli/src/factory/composition.ts docs/template/contracts-upgrade.md
  rtk git commit -m "feat: add atomic contracts audit upgrade"
  ```
...
### Task 19: Pilot One Real Maestro Brain Journey

**Class:** `fixture-to-real`  
**PRs:** `M1a` through `M1d`, then admission-only `M2`  
**Depends on:** `W1` plus the reviewed alpha.3 release candidate containing
`F1`, `F2`, `B1`, `U1`, and `R1`. The pilot validates that candidate before Task
20 seals it; production adoption uses the resulting sealed release.
...
- [ ] **Step 1 (`M1a`): Install in audit mode and add only an assembling
      contract.** Run `maestro upgrade --contracts-audit`, review the frozen
...
- [ ] **Step 2 (`M1b`): Register and implement UI authority while dark.** Add
...
- [ ] **Step 3 (`M1c`): Register and implement CLI authority while dark.** Add
...
- [ ] **Step 4 (`M1d`): Implement trusted steps and make a local temporary flip
...
- [ ] **Step 5: Merge `M1a` through `M1d` bottom-up through GitHub.** Each pull
...
- [ ] **Step 6 (`M2`): Create the current-main admission-only pull request.**
      From protected main after all four slices merge, change only
      `@assembling -> @admitted` plus a narrowly scoped integration correction
      if the complete current-main execution exposes one. The Feature must
      already exist assembling on the immutable protected base. Use merge-queue
      batch size one.
...
### Task 20: Delete Superseded Machinery And Seal `v0.2.0-alpha.3`

**Class:** `fixture-to-real`  
**PR:** `D1`  
**Depends on:** `F1`, `F2`, `B1`, `U1`, `R1`, `W1`, the green 28-case mutation
gauntlet, and a green Brain admission pilot. Do not perform any deletion on a
weaker evidence set.
...
- [ ] **Step 4: Re-run the pristine reference and all 28 mutations.**
  ...
  Expected: pristine generated product passes and all 28 false-green changes
  create their specific red findings after deletion.

- [ ] **Step 5: Run the complete template gate before sealing.**

  Run after commit on the remote worker:
  `rtk maestro-remote-test -- pnpm verify`

  Run the protected Woodpecker test pull request through merge queue.

  Expected: `ci/woodpecker/pr/verify` is green for the exact merge candidate;
  Qlty is advisory; no Buildkite/Fabro/Graphite authority is invoked.

- [ ] **Step 6: Seal, inspect, and exercise the immutable release.** Run the
      existing release sealer for `v0.2.0-alpha.3`, verify the generated
      manifest contains
      Cucumber/config/auth/controller/public-surface/upgrade/release assets, and
      update create composition to the new immutable tag/checksums. Never modify
      alpha.2.
  ...
  preview/write, run the factory-only reference acceptance overlay, and run a
  contracts-audit upgrade fixture.

- [ ] **Step 7: Commit the deletion and sealed release.**

  ```bash
  rtk git add -A packages/product-journey tooling/quality/check-product-journeys.mts tooling/quality/check-product-journeys.test.mts docs/superpowers/specs/2026-08-01-product-journey-admission-design.md docs/superpowers/plans/2026-08-01-product-journey-admission-and-brain-completion.md package.json Justfile tooling/quality/src/check-definitions.mts tooling/generators/src/blueprints/saasRegistrationProjections.ts apps/cli/src releases/v0.2.0-alpha.3 docs/template/app-factory-guide.md docs/template/customer-target-contract.md
...
| -------------------------------------------------------- | ---------------- |
| Feature is sole behavioral contract                      | 1, 7, 13, 15, 20 |
...
| Complete mutation gauntlet                               | 12, 17, 18, 20   |
| Custom/fake machinery deleted after proof                | 14, 20           |
| Real Brain UI/CLI journey                                | 19               |
