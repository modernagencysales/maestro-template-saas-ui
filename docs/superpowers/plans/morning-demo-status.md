...
| Social        | branch-diagnosed-ci-red-hosted-auth-failed | Merged `1fa00df0` remains red/unaccepted; clean pushed diagnostic branch `eb918d04` exposes missing claim token and retains a statically green shard-C readiness repair          | Owner fix/defer/remove decision after the two-cycle boundary            | Shard B loses its one-time token; shard C is not runtime-accepted; Railway callback remains invalid; no third scenario, CI, deploy, or auth retry authorized                    |
...
| Focused tests | terminal-social-diagnosis-sealed           | Pipeline `345` failure plus exact branch-only diagnosis `eb918d04` are sealed; focused readiness contracts and static policy checks passed                                       | Owner decision before any third Social runtime cycle                    | Shard B remains unfixed and shard C remains runtime-unaccepted                                                                                                                  |
