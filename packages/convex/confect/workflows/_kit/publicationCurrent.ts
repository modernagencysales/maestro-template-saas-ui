      ...
      ...(release.subworkflowRuntime
        ? {
            subworkflowRuntime: {
              ...
              ...(release.subworkflowRuntime.boundedBatch
                ? {
                    boundedBatch: {
                      selectItems: {
                        module:
                          release.subworkflowRuntime.boundedBatch.selectItems
                            .module,
                        exportName:
                          release.subworkflowRuntime.boundedBatch.selectItems
                            .exportName,
                      ...
                      mapBatchArgs: {
                        module:
                          release.subworkflowRuntime.boundedBatch.mapBatchArgs
                            .module,
                        exportName:
                          release.subworkflowRuntime.boundedBatch.mapBatchArgs
                            .exportName,
...
export const defineWorkflowRelease = <RunnerRef, CompletionRef>(
  ...
  for (const binding of release.subworkflowBindings) {
    if (
      binding.maxChildStarts !== undefined &&
      (!Number.isSafeInteger(binding.maxChildStarts) ||
        binding.maxChildStarts < 1)
    ) {
      throw new Error(
        "Workflow subworkflow max child starts must be a positive safe integer",
      );
    }
  }
...
const validateSubworkflowRuntimeBinding = (
  ...
  const boundedBatch = binding.boundedBatch;
  if (
    boundedBatch !== undefined &&
    (boundedBatch.selectItems.module.length === 0 ||
      boundedBatch.selectItems.exportName.length === 0 ||
      typeof boundedBatch.selectItems.selectItems !== "function" ||
      boundedBatch.selectItems.selectItems.name !==
        boundedBatch.selectItems.exportName ||
      boundedBatch.mapBatchArgs.module.length === 0 ||
      boundedBatch.mapBatchArgs.exportName.length === 0 ||
      boundedBatch.mapBatchArgs.schemaName.length === 0 ||
      typeof boundedBatch.mapBatchArgs.mapBatchArgs !== "function" ||
      boundedBatch.mapBatchArgs.mapBatchArgs.name !==
        boundedBatch.mapBatchArgs.exportName)
  ) {
    throw new Error(
      "Workflow bounded batch runtime exports do not match their immutable descriptors",
    );
  }
  let graph: unknown;
  try {
  ...
  if (
    ...
    (boundedBatch !== undefined &&
      (!authority.sourceClosure.modules.some(
        ({ path }) => path === boundedBatch.selectItems.module,
      ) ||
        !authority.sourceClosure.modules.some(
          ({ path }) => path === boundedBatch.mapBatchArgs.module,
        ) ||
        boundedBatch.mapBatchArgs.schemaName !==
          binding.argumentMapper.schemaName))
  ) {
    throw new Error(
    ...
    ...(boundedBatch
      ? {
          boundedBatch: Object.freeze({
            selectItems: Object.freeze({ ...boundedBatch.selectItems }),
            mapBatchArgs: Object.freeze({ ...boundedBatch.mapBatchArgs }),
          }),
        }
      : {}),
  });
