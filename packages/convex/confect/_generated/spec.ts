...
const spec: Spec.Spec<
  ...
  | GroupSpec.NamedAt<GroupSpec.GroupSpec<"Convex", "workflowContracts", never, GroupSpec.NamedAt<typeof workflowContracts_publicationFixture, "publicationFixture">>, "workflowContracts">
...
> = Spec.make().addAt("access", GroupSpec.makeAt("access").addGroupAt("invitations", access_invitations).addGroupAt("members", access_members).addGroupAt("provisioning", access_provisioning)).addAt("agents", GroupSpec.makeAt("agents").addGroupAt("assistant", agents_assistant)).addAt("auth", GroupSpec.makeAt("auth").addGroupAt("workspaces", auth_workspaces)).addAt("brain", GroupSpec.makeAt("brain").addGroupAt("pages", brain_pages)).addAt("capabilities", GroupSpec.makeAt("capabilities").addGroupAt("_versions", GroupSpec.makeAt("_versions").addGroupAt("publicationEcho", GroupSpec.makeAt("publicationEcho").addGroupAt("v1", capabilities__versions_publicationEcho_v1))).addGroupAt("catalog", capabilities_catalog).addGroupAt("sourceGroundedBrief", capabilities_sourceGroundedBrief)).addAt("demo", GroupSpec.makeAt("demo").addGroupAt("showcase", demo_showcase)).addAt("editorSync", editorSync).addAt("jobs", GroupSpec.makeAt("jobs").addGroupAt("workpool", jobs_workpool)).addAt("ops", GroupSpec.makeAt("ops").addGroupAt("actions", ops_actions).addGroupAt("billing", ops_billing).addGroupAt("coediting", ops_coediting).addGroupAt("dataLifecycle", ops_dataLifecycle).addGroupAt("flags", ops_flags).addGroupAt("health", ops_health).addGroupAt("knowledge", ops_knowledge).addGroupAt("notifications", ops_notifications).addGroupAt("transforms", ops_transforms).addGroupAt("versioning", ops_versioning)).addAt("workflowContracts", GroupSpec.makeAt("workflowContracts").addGroupAt("publicationFixture", workflowContracts_publicationFixture)).addAt("workflowRunners", GroupSpec.makeAt("workflowRunners").addGroupAt("publicationFixture", GroupSpec.makeAt("publicationFixture").addGroupAt("v1", workflowRunners_publicationFixture_v1))).addAt("workflows", GroupSpec.makeAt("workflows").addGroupAt("effectReservations", workflows_effectReservations).addGroupAt("eventInstances", workflows_eventInstances).addGroupAt("lifecycle", workflows_lifecycle).addGroupAt("stageObservations", workflows_stageObservations).addGroupAt("subworkflowLinks", workflows_subworkflowLinks));
