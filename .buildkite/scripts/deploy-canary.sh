...
case "${MODE}" in
  backend)
    ...
    export CHECKED_AT
    HEALTH_ARGS="$(node -e 'process.stdout.write(JSON.stringify({environment:"live",commitSha:process.env.BUILDKITE_COMMIT,checkedAt:Number(process.env.CHECKED_AT)}))')"
